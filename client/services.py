...
def get_data(user):
    ...
    if user_id == 1:
        for client in User.objects.all():
            ...
            if client_id == 1:
                continue
            airconds_count = Airconddata.objects.filter(client_d=client.id).first().airconds_count
    ...
    airconds_count = Airconddata.objects.filter(client_d=user_id).first().airconds_count
    for i in range(1, airconds_count * 2 + 1):
