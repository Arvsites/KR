...
def get_data(user):
    ...
    try:
        airconds_count = Airconddata.objects.filter(client_login_id=user_id).first().airconds_count
    ...
    if user_id == 1:
        for client in User.objects.all():
            if client.id == 1:
                continue
            airconds_count = Airconddata.objects.filter(client_login_id=client.id).first().airconds_count
